...
df['sdg_7_dummy'] = (df['sdg'] == 7) & (df['labels_positive'] > df['labels_negative']) & (df['agreement'] > 0.75)

# Convert boolean values to 1 or 0
df['sdg_7_dummy'] = df['sdg_7_dummy'].astype(int)

# Display the updated DataFrame
print(df)

# Count the occurrences of new_dummy_variable = 1
count_sdg7 = df['sdg_7_dummy'].value_counts()[1]
count_sdg7_keywords = keyword_data['SDGs'].value_counts()['SDG7']

# Display the count
print("Count of sdg7 texts = 1:", count_sdg7) #1101
print("Count of sdg7 keywords = 1:", count_sdg7_keywords) #717

#For SDG1
sdg7_keywords = keyword_data[keyword_data['SDGs'] == "SDG7"]['Words (Phrases)'].tolist()

for keyword in sdg7_keywords:
...
for keyword in sdg7_keywords:
    ...
    if keyword in df_sdg7.columns and df_sdg7[keyword].any():
        columns_with_ones_sdg7.append(keyword)

# Print the columns with at least one "1" value for texts labelled as sdg1
print(columns_with_ones_sdg7)

# List to store the columns to delete
columns_to_delete_sdg7 = []
...
for keyword in sdg7_keywords:
    ...
    if keyword in df.columns and not df.loc[df['sdg_7_dummy'] == 1, keyword].any():
        columns_to_delete_sdg7.append(keyword)

# Delete the columns with only "0" values for SDG1 labelled texts
df = df.drop(columns=columns_to_delete_sdg7)

# Print the remaining columns
print(df.columns)

# Assign the feature and target variables
features = df.iloc[:, -502:]
target = df['sdg_7_dummy']
'''
...
print(coefficients)
coefficients.to_csv('coefficients_sdg7_ridge.csv', index=False)
