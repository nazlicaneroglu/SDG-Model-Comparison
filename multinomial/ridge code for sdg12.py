...
df['sdg_12_dummy'] = (df['sdg'] == 12) & (df['labels_positive'] > df['labels_negative']) & (df['agreement'] > 0.75)

# Convert boolean values to 1 or 0
df['sdg_12_dummy'] = df['sdg_12_dummy'].astype(int)

# Display the updated DataFrame
print(df)

# Count the occurrences of new_dummy_variable = 1
count_sdg12 = df['sdg_12_dummy'].value_counts()[1]
count_sdg12_keywords = keyword_data['SDGs'].value_counts()['SDG12']

# Display the count
print("Count of sdg12 texts = 1:", count_sdg12) #1101
print("Count of sdg12 keywords = 1:", count_sdg12_keywords) #717

#For SDG1
sdg12_keywords = keyword_data[keyword_data['SDGs'] == "SDG12"]['Words (Phrases)'].tolist()

for keyword in sdg12_keywords:
...
for keyword in sdg12_keywords:
    ...
    if keyword in df_sdg12.columns and df_sdg12[keyword].any():
        columns_with_ones_sdg12.append(keyword)

# Print the columns with at least one "1" value for texts labelled as sdg1
print(columns_with_ones_sdg12)

# List to store the columns to delete
columns_to_delete_sdg12 = []
...
for keyword in sdg12_keywords:
    ...
    if keyword in df.columns and not df.loc[df['sdg_12_dummy'] == 1, keyword].any():
        columns_to_delete_sdg12.append(keyword)

# Delete the columns with only "0" values for SDG1 labelled texts
df = df.drop(columns=columns_to_delete_sdg12)

# Print the remaining columns
print(df.columns)

# Assign the feature and target variables
features = df.iloc[:, -len(columns_with_ones_sdg12):]
target = df['sdg_12_dummy']
'''
...
print(coefficients)
coefficients.to_csv('coefficients_sdg12_ridge.csv', index=False)
