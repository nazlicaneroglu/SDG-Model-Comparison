...
df['sdg_6_dummy'] = (df['sdg'] == 6) & (df['labels_positive'] > df['labels_negative']) & (df['agreement'] > 0.75)

# Convert boolean values to 1 or 0
df['sdg_6_dummy'] = df['sdg_6_dummy'].astype(int)

# Display the updated DataFrame
print(df)

# Count the occurrences of new_dummy_variable = 1
count_sdg6 = df['sdg_6_dummy'].value_counts()[1]
count_sdg6_keywords = keyword_data['SDGs'].value_counts()['SDG6']

# Display the count
print("Count of sdg6 texts = 1:", count_sdg6) #1101
print("Count of sdg6 keywords = 1:", count_sdg6_keywords) #717

#For SDG1
sdg6_keywords = keyword_data[keyword_data['SDGs'] == "SDG6"]['Words (Phrases)'].tolist()

for keyword in sdg6_keywords:
...
for keyword in sdg6_keywords:
    ...
    if keyword in df_sdg6.columns and df_sdg6[keyword].any():
        columns_with_ones_sdg6.append(keyword)

# Print the columns with at least one "1" value for texts labelled as sdg1
print(columns_with_ones_sdg6)

# List to store the columns to delete
columns_to_delete_sdg6 = []
...
for keyword in sdg6_keywords:
    ...
    if keyword in df.columns and not df.loc[df['sdg_6_dummy'] == 1, keyword].any():
        columns_to_delete_sdg6.append(keyword)

# Delete the columns with only "0" values for SDG1 labelled texts
df = df.drop(columns=columns_to_delete_sdg6)

# Print the remaining columns
print(df.columns)

# Assign the feature and target variables
features = df.iloc[:, -612:]
target = df['sdg_6_dummy']
'''
...
print(coefficients)
coefficients.to_csv('coefficients_sdg6_ridge.csv', index=False)
