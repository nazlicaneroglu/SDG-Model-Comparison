...
df['sdg_10_dummy'] = (df['sdg'] == 10) & (df['labels_positive'] > df['labels_negative']) & (df['agreement'] > 0.75)

# Convert boolean values to 1 or 0
df['sdg_10_dummy'] = df['sdg_10_dummy'].astype(int)

# Display the updated DataFrame
print(df)

# Count the occurrences of new_dummy_variable = 1
count_sdg10 = df['sdg_10_dummy'].value_counts()[1]
count_sdg10_keywords = keyword_data['SDGs'].value_counts()['SDG10']

# Display the count
print("Count of sdg10 texts = 1:", count_sdg10) #1101
print("Count of sdg10 keywords = 1:", count_sdg10_keywords) #717

#For SDG1
sdg10_keywords = keyword_data[keyword_data['SDGs'] == "SDG10"]['Words (Phrases)'].tolist()

for keyword in sdg10_keywords:
...
for keyword in sdg10_keywords:
    ...
    if keyword in df_sdg10.columns and df_sdg10[keyword].any():
        columns_with_ones_sdg10.append(keyword)

# Print the columns with at least one "1" value for texts labelled as sdg1
print(columns_with_ones_sdg10)

# List to store the columns to delete
columns_to_delete_sdg10 = []
...
for keyword in sdg10_keywords:
    ...
    if keyword in df.columns and not df.loc[df['sdg_10_dummy'] == 1, keyword].any():
        columns_to_delete_sdg10.append(keyword)

# Delete the columns with only "0" values for SDG1 labelled texts
df = df.drop(columns=columns_to_delete_sdg10)

# Print the remaining columns
print(df.columns)

# Assign the feature and target variables
features = df.iloc[:, -259:]
target = df['sdg_10_dummy']
'''
...
print(coefficients)
coefficients.to_csv('coefficients_sdg10_ridge.csv', index=False)
