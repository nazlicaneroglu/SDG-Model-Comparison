...
df['sdg_8_dummy'] = (df['sdg'] == 8) & (df['labels_positive'] > df['labels_negative']) & (df['agreement'] > 0.75)

# Convert boolean values to 1 or 0
df['sdg_8_dummy'] = df['sdg_8_dummy'].astype(int)

# Display the updated DataFrame
print(df)

# Count the occurrences of new_dummy_variable = 1
count_sdg8 = df['sdg_8_dummy'].value_counts()[1]
count_sdg8_keywords = keyword_data['SDGs'].value_counts()['SDG8']

# Display the count
print("Count of sdg8 texts = 1:", count_sdg8) #1101
print("Count of sdg8 keywords = 1:", count_sdg8_keywords) #717

#For SDG1
sdg8_keywords = keyword_data[keyword_data['SDGs'] == "SDG8"]['Words (Phrases)'].tolist()

for keyword in sdg8_keywords:
...
for keyword in sdg8_keywords:
    ...
    if keyword in df_sdg8.columns and df_sdg8[keyword].any():
        columns_with_ones_sdg8.append(keyword)

# Print the columns with at least one "1" value for texts labelled as sdg1
print(columns_with_ones_sdg8)

# List to store the columns to delete
columns_to_delete_sdg8 = []
...
for keyword in sdg8_keywords:
    ...
    if keyword in df.columns and not df.loc[df['sdg_8_dummy'] == 1, keyword].any():
        columns_to_delete_sdg8.append(keyword)

# Delete the columns with only "0" values for SDG1 labelled texts
df = df.drop(columns=columns_to_delete_sdg8)

# Print the remaining columns
print(df.columns)

# Assign the feature and target variables
features = df.iloc[:, -373:]
target = df['sdg_8_dummy']
'''
...
print(coefficients)
coefficients.to_csv('coefficients_sdg8_ridge.csv', index=False)
