...
df['sdg_9_dummy'] = (df['sdg'] == 9) & (df['labels_positive'] > df['labels_negative']) & (df['agreement'] > 0.75)

# Convert boolean values to 1 or 0
df['sdg_9_dummy'] = df['sdg_9_dummy'].astype(int)

# Display the updated DataFrame
print(df)

# Count the occurrences of new_dummy_variable = 1
count_sdg9 = df['sdg_9_dummy'].value_counts()[1]
count_sdg9_keywords = keyword_data['SDGs'].value_counts()['SDG9']

# Display the count
print("Count of sdg9 texts = 1:", count_sdg9) #1101
print("Count of sdg9 keywords = 1:", count_sdg9_keywords) #717

#For SDG1
sdg9_keywords = keyword_data[keyword_data['SDGs'] == "SDG9"]['Words (Phrases)'].tolist()

for keyword in sdg9_keywords:
...
for keyword in sdg9_keywords:
    ...
    if keyword in df_sdg9.columns and df_sdg9[keyword].any():
        columns_with_ones_sdg9.append(keyword)

# Print the columns with at least one "1" value for texts labelled as sdg1
print(columns_with_ones_sdg9)

# List to store the columns to delete
columns_to_delete_sdg9 = []
...
for keyword in sdg9_keywords:
    ...
    if keyword in df.columns and not df.loc[df['sdg_9_dummy'] == 1, keyword].any():
        columns_to_delete_sdg9.append(keyword)

# Delete the columns with only "0" values for SDG1 labelled texts
df = df.drop(columns=columns_to_delete_sdg9)

# Print the remaining columns
print(df.columns)

# Assign the feature and target variables
features = df.iloc[:, -440:]
target = df['sdg_9_dummy']
'''
...
print(coefficients)
coefficients.to_csv('coefficients_sdg9_ridge.csv', index=False)
