...
df['sdg_5_dummy'] = (df['sdg'] == 5) & (df['labels_positive'] > df['labels_negative']) & (df['agreement'] > 0.75)

# Convert boolean values to 1 or 0
df['sdg_5_dummy'] = df['sdg_5_dummy'].astype(int)

# Display the updated DataFrame
print(df)

# Count the occurrences of new_dummy_variable = 1
count_sdg5 = df['sdg_5_dummy'].value_counts()[1]
count_sdg5_keywords = keyword_data['SDGs'].value_counts()['SDG5']

# Display the count
print("Count of sdg5 texts = 1:", count_sdg5) #1101
print("Count of sdg5 keywords = 1:", count_sdg5_keywords) #717

#For SDG1
sdg5_keywords = keyword_data[keyword_data['SDGs'] == "SDG5"]['Words (Phrases)'].tolist()

# For each SDG1 related keyword
for keyword in sdg5_keywords:
...
for keyword in sdg5_keywords:
    ...
    if keyword in df_sdg5.columns and df_sdg5[keyword].any():
        columns_with_ones_sdg5.append(keyword)

# Print the columns with at least one "1" value for texts labelled as sdg1
print(columns_with_ones_sdg5)

# List to store the columns to delete
columns_to_delete_sdg5 = []
...
for keyword in sdg5_keywords:
    ...
    if keyword in df.columns and not df.loc[df['sdg_5_dummy'] == 1, keyword].any():
        columns_to_delete_sdg5.append(keyword)

# Delete the columns with only "0" values for SDG1 labelled texts
df = df.drop(columns=columns_to_delete_sdg5)

# Print the remaining columns
print(df.columns)

# Assign the feature and target variables
features = df.iloc[:, -478:]
target = df['sdg_5_dummy']
'''
...
print(coefficients)
coefficients.to_csv('coefficients_sdg5_ridge.csv', index=False)
