...
df['sdg_14_dummy'] = (df['sdg'] == 14) & (df['labels_positive'] > df['labels_negative']) & (df['agreement'] > 0.75)

# Convert boolean values to 1 or 0
df['sdg_14_dummy'] = df['sdg_14_dummy'].astype(int)

# Display the updated DataFrame
print(df)

# Count the occurrences of new_dummy_variable = 1
count_sdg14 = df['sdg_14_dummy'].value_counts()[1]
count_sdg14_keywords = keyword_data['SDGs'].value_counts()['SDG14']

# Display the count
print("Count of sdg14 texts = 1:", count_sdg14) #1101
print("Count of sdg14 keywords = 1:", count_sdg14_keywords) #717

#For SDG1
sdg14_keywords = keyword_data[keyword_data['SDGs'] == "SDG14"]['Words (Phrases)'].tolist()

for keyword in sdg14_keywords:
...
for keyword in sdg14_keywords:
    ...
    if keyword in df_sdg14.columns and df_sdg14[keyword].any():
        columns_with_ones_sdg14.append(keyword)

# Print the columns with at least one "1" value for texts labelled as sdg1
print(columns_with_ones_sdg14)

# List to store the columns to delete
columns_to_delete_sdg14 = []
...
for keyword in sdg14_keywords:
    ...
    if keyword in df.columns and not df.loc[df['sdg_14_dummy'] == 1, keyword].any():
        columns_to_delete_sdg14.append(keyword)

# Delete the columns with only "0" values for SDG1 labelled texts
df = df.drop(columns=columns_to_delete_sdg14)

# Print the remaining columns
print(df.columns)

# Assign the feature and target variables
features = df.iloc[:, -len(columns_with_ones_sdg14):]
target = df['sdg_14_dummy']
'''
...
print(coefficients)
coefficients.to_csv('coefficients_sdg14_ridge.csv', index=False)
